...
export function generateStaticParams() {
  return competitiveProgramming.map((topic) => ({
    slug: topic.slug,
  }));
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const topic = competitiveProgramming.find((t) => t.slug === slug);
...
export default async function TechniqueTopicPage({ params }: PageProps) {
  const { slug } = await params;
  const topic = competitiveProgramming.find((t) => t.slug === slug);
  ...
  return (
    <>
      <JsonLd 
        ...
        name={topic.title} 
        description={topic.seo.description}
        url={`${siteConfig.domain}/competitive-programming/techniques/${topic.slug}`}
      />
      <JsonLd 
        type="BreadcrumbList"
        breadcrumbs={[
          ...
          { name: "Competitive Programming", item: `${siteConfig.domain}/competitive-programming` },
          { name: topic.title, item: `${siteConfig.domain}/competitive-programming/techniques/${topic.slug}` }
        ]}
      />
      <Navbar />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
              ...
              <div className="lg:col-span-3 space-y-12">
                ...
                {topic.whenToUse && topic.whenToUse.length > 0 && (
                  <div>
                    <h2 className="text-2xl font-medium text-foreground mb-4">When to Use</h2>
                    <ul className="list-disc pl-5 space-y-2 text-foreground/80 leading-relaxed">
                      {topic.whenToUse.map((point, i) => (
                        <li key={i}>{point}</li>
                      ))}
                    </ul>
                  </div>
                ...
                {topic.howToIdentify && (
                  <div>
                    <h2 className="text-2xl font-medium text-foreground mb-4">How to Identify</h2>
                    <div className="prose max-w-none text-foreground/80 leading-relaxed">
                      {topic.howToIdentify}
                    </div>
                  </div>
                ...
                {topic.codeExamples && topic.codeExamples.length > 0 && (
                  <div>
                    <h2 className="text-2xl font-medium text-foreground mb-4">Template / Implementation</h2>
