...
// Data
import { competitiveProgrammingHubData as data } from "@/data/competitive-programming/hub-data";

export const metadata: Metadata = {
  title: "Competitive Programming Ecosystem | prashanta.dev",
      ...
      <main className="pt-header">
        <Container>
              ...
              {data.topicCategories.map((category) => (
                <div key={category.id} className="scroll-mt-32" id={category.id}>
                  ...
                  {category.topics.length > 0 && (
                    <div className="mb-8">
                      <FeaturedTopic topic={category.topics[0]} href={`/competitive-programming/techniques/${category.topics[0].slug}`} />
                    </div>
                  )}
                  ...
                  {category.topics.length > 1 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {category.topics.slice(1).map((topic) => (
                        <Link 
                          key={topic.id} 
                          href={`/competitive-programming/techniques/${topic.slug}`}
          ...
          <Section className="py-20 text-center">
            <h3 className="text-2xl font-bold text-foreground mb-6">Ready to solve real-world problems?</h3>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/problems" className="px-8 py-4 bg-surface border border-border rounded-lg hover:border-accent/50 transition-colors font-medium">
