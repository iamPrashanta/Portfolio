...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  // @ts-ignore - Handle dual types during transition
  const seoTitle = topic.seo?.title || topic.title;
  // @ts-ignore
  const seoDesc = topic.seo?.description || topic.shortDescription;
  
  return {
    title: `${seoTitle} | prashanta.dev`,
    description: seoDesc,
    // @ts-ignore
...
export default async function DataStructureTopicPage({ params }: PageProps) {
  ...
  // @ts-ignore - Handle dual types during transition
  const isDeepTopic = Boolean(topic.overview || topic.mentalModel);
  ...
  return (
    <>
      ...
      <main className="pt-header">
        {isDeepTopic ? (
          <DeepTopicLayout 
            // @ts-ignore - Transitioning types
            topic={topic} 
            // @ts-ignore
            nextTopics={(topic.nextTopics || []).map(nextSlug => {
              const found = dataStructures.find(t => t.slug === nextSlug);
              if (found) {
                return {
                  slug: found.slug,
                  title: found.title,
                  description: found.shortDescription,
                  href: `/data-structures/${found.slug}`
                };
              }
              return {
                slug: nextSlug,
