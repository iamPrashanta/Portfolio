...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const topic = algorithms.find((t) => t.slug === slug);
...
export default async function AlgorithmTopicPage({ params }: PageProps) {
  const { slug } = await params;
  const topic = algorithms.find((t) => t.slug === slug);
  ...
  return (
    <>
      <JsonLd 
        ...
        name={topic.title} 
        description={topic.seo.description}
        url={`${siteConfig.domain}/algorithms/${topic.slug}`}
      />
      <JsonLd 
        type="BreadcrumbList"
        breadcrumbs={[
          ...
          { name: "Algorithms", item: `${siteConfig.domain}/algorithms` },
          { name: topic.title, item: `${siteConfig.domain}/algorithms/${topic.slug}` }
        ]}
      />
      <Navbar />
