...
// Data
import { algorithmsHubData as data } from "@/data/algorithms/hub-data";

export const metadata: Metadata = {
  title: "Algorithms Ecosystem | prashanta.dev",
      ...
      <main className="pt-header">
        <Container>
              ...
              {data.topicCategories.map((category) => (
                <div key={category.id} className="scroll-mt-32" id={category.id}>
                  ...
                  {category.topics.length > 0 && (
                    <div className="mb-8">
                      <FeaturedTopic topic={category.topics[0]} href={`/algorithms/${category.topics[0].slug}`} />
                    </div>
                  )}
          ...
          <Section className="py-20 text-center">
            <h3 className="text-2xl font-bold text-foreground mb-6">Ready for the ultimate challenge?</h3>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/competitive-programming" className="px-8 py-4 bg-surface border border-border rounded-lg hover:border-accent/50 transition-colors font-medium">
