...
// Data
import { computerScienceHubData as data } from "@/data/computer-science/hub-data";

export const metadata: Metadata = {
  title: "Computer Science Ecosystem | prashanta.dev",
      ...
      <main className="pt-header">
        <Container>
            ...
            <div className="space-y-32">
              {data.topicCategories.map((category) => (
                <div key={category.id} className="scroll-mt-32" id={category.id}>
                  ...
                  {/* We feature the very first topic of each category to break up the grid */}
                  {category.topics.length > 0 && (
                    <div className="mb-8">
                      <FeaturedTopic topic={category.topics[0]} href={`/computer-science/${category.id}/${category.topics[0].slug}`} />
                    </div>
                  )}

                  {/* Grid for the rest of the topics */}
                  {category.topics.length > 1 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {category.topics.slice(1).map((topic) => (
                        <Link 
                          key={topic.id} 
                          href={`/computer-science/${category.id}/${topic.slug}`}
          ...
          <Section className="py-20 text-center">
            <h3 className="text-2xl font-bold text-foreground mb-6">Ready for the next domain?</h3>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link href="/data-structures" className="px-8 py-4 bg-surface border border-border rounded-lg hover:border-accent/50 transition-colors font-medium">
                Explore Data Structures →
              </Link>
              <Link href="/algorithms" className="px-8 py-4 bg-surface border border-border rounded-lg hover:border-accent/50 transition-colors font-medium">
                Explore Algorithms →
              </Link>
            </div>
          </Section>
