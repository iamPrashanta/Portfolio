...
export default async function CareerDetailPage({ params }: Props) {
  ...
  return (
    <>
      <Navbar />
      <main className="pt-header bg-white min-h-screen">
        <Section size="lg" className="pt-8">
          <Container size="default">
            ...
            <div className="mb-12 animate-fade-up">
              <Link 
                href="/careers" 
                className="inline-flex items-center text-sm font-medium text-muted hover: text-foreground:text-white transition-colors group"
              >
            ...
            <div className="max-w-4xl mb-16 animate-fade-up" style={{ animationDelay: "0.1s" }}>
              ...
              <h1 className="text-[3rem] md:text-[4.5rem] font-medium leading-tight mb-8 text-foreground">
                {career.title}
              </h1>
            ...
            {/* Role Meta Data */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 py-12 border-t border-b border-black/10 mb-16 animate-fade-up" style={{ animationDelay: "0.2s" }}>
              <div>
                <h5 className="text-sm font-medium text-muted uppercase tracking-wider mb-2">Department</h5>
                <p className="text-lg font-medium text-foreground">{career.department}</p>
              </div>
              <div>
                <h5 className="text-sm font-medium text-muted uppercase tracking-wider mb-2">Engagement</h5>
                <p className="text-lg font-medium text-foreground">{career.engagement}</p>
              </div>
              <div>
                <h5 className="text-sm font-medium text-muted uppercase tracking-wider mb-2">Location</h5>
                <p className="text-lg font-medium text-foreground">{career.location}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24 mb-24 animate-fade-up" style={{ animationDelay: "0.3s" }}>
              <div className="lg:col-span-8">
                <h2 className="text-3xl font-medium mb-8 text-foreground">About the role</h2>
                
                <h3 className="text-xl font-medium mb-4 text-foreground">Responsibilities</h3>
                ...
                <h3 className="text-xl font-medium mb-4 text-foreground">Requirements</h3>
                <ul className="space-y-4 mb-12">
                ...
                {career.niceToHave && career.niceToHave.length > 0 && (
                  <>
                    <h3 className="text-xl font-medium mb-4 text-foreground">Nice to have</h3>
                    <ul className="space-y-4 mb-12">
                      {career.niceToHave.map((nice, idx) => (
                        <li key={idx} className="flex items-start">
                          <div className="w-1.5 h-1.5 rounded-full bg-neutral-300 mt-2.5 mr-4 flex-shrink-0" />
              ...
              <div className="lg:col-span-4">
                {career.technologies && career.technologies.length > 0 && (
                  <div className="bg-neutral-50 rounded-2xl p-8 sticky top-32">
                    <h3 className="text-xl font-medium mb-6 text-foreground">Technologies</h3>
                    <div className="flex flex-wrap gap-3">
                      {career.technologies.map((tech, idx) => (
                        <span key={idx} className="px-4 py-2 bg-white text-foreground rounded-full text-sm font-medium border border-black/5">
                          {tech}
                        </span>
                      ))}
                    </div>
            ...
            {/* Footer CTA */}
            <div className="p-12 lg:p-16 rounded-[32px] bg-neutral-100 border border-transparent text-center animate-fade-up">
              <h2 className="text-3xl lg:text-4xl font-medium mb-6 text-foreground">Interested in collaborating?</h2>
